...
def remove_seams(height,seam_paths, matrix):
    for y in range(height):
        #seleccionem els punts minims a eliminar en la fila y
        path = []
        for p in seam_paths:
            path.append(p[y])
	#ordenem els paths y els voltemjem per q eue comense a eliminar per la dreta
...
def dp_seam_carving_multi(grad,mat,N,pscore=0.8):
    ...
    cost_min=sorted(aux_list)
    millor=cost_min[0][0]

    #se quedem en els punts on estan els valor que siguen menor que el ratio, la resta els descartem
    aux_list=[]
    for x in cost_min:
        if(x[0]*pscore<millor):
            #afegim el punt 
            aux_list.append(x[1])
        else:
        	#els altre punts ja no son valids per al ratio
            break

    cost_min=aux_list
    paths = []
    utilizat=set()
    #Mentre no aplegem a N camins i ens quedem camins per procesar anem eliminan i afegint-los als paths
    while(len(paths)<N and len(cost_min)>0):
        pm=cost_min.pop(0)
        path = [pm]
        aux_l_path=set()
        for y in range(height-2,-1,-1):
            #busquem el punt on el valor es el minim de la fila superior, rang (y-1)(x-1 x x+1)
            aux_list=[]
            for x in range(pm-1,pm+2):
                aux_list.append((mat[y][x],x))
            vm,pm=min(aux_list)
            #comprobem si aquest punt ja esta en algun cami y en aques cas abortem aquesta solucio
            if((pm,y) in utilizat):
                break
            #afegim el nou punt en el set de utilizats
            aux_l_path.add((pm,y))
            path.append(pm)

        #comprobem que el path siga de la talla de l'altura de cas contrari es perq el cami no era valid i no tots els punts estan en el path
        if(len(path)==height):
            #com el path es correcte i no intersecta amb cap altre l'introduim en el set de punts utilizats
            utilizat=utilizat.union(aux_l_path)
            path.reverse()
            paths.append(path)    


    print("Paths a eliminar->",len(paths))
    # COMPLETE HERE
    return paths
