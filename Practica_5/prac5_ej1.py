...
def BranchAndBound(G):
  ...
  def optimistic(cost,s):
    ...
    if cost==None:
    	...
    	#tenim el cost del pare en cost, y ara li restem dos vegades el valor del fill
    	#que abans haviem sumat
	    opt+=cost-(2*sum(G[s[-1]][x] for x in s[:-1]))
  ...
  fx = optimistic(None,x) # equivale a evaluate quando is_complete(x)

  # anyadimos el estado inicial:
  s = []
  cost=None
  opt = optimistic(cost,s)
